//! Medium-term momentum signal based on 6-month returns.

use polars::prelude::*;
use serde::{Deserialize, Serialize};
use tarifa_traits::{Date, MarketData, Result, Signal, TarifaError};

/// Configuration for medium-term momentum signal.
///
/// This signal computes cumulative returns over a medium lookback period,
/// typically 6 months (126 trading days).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MediumTermMomentumConfig {
    /// Number of trading days to look back (default: 126 days ≈ 6 months)
...
/// Medium-term momentum signal.
///
/// Computes 6-month cumulative returns and applies cross-sectional standardization.
/// This time horizon captures intermediate-term price trends.
///
/// # Example
///
/// ```ignore
/// use tarifa_signals::momentum::MediumTermMomentum;
///
/// let signal = MediumTermMomentum::default();
...
impl MediumTermMomentum {
    /// Create a new medium-term momentum signal with the given configuration.
    #[must_use]
    pub const fn new(config: MediumTermMomentumConfig) -> Self {
        Self { config }
    }
...
impl Default for MediumTermMomentum {
    fn default() -> Self {
        Self::new(MediumTermMomentumConfig::default())
    }
}

impl Signal for MediumTermMomentum {
    fn name(&self) -> &str {
...
mod tests {
    ...
    fn test_default_config() {
        let config = MediumTermMomentumConfig::default();
        assert_eq!(config.lookback_days, 126);
    }

    #[test]
    fn test_custom_config() {
        let config = MediumTermMomentumConfig { lookback_days: 150 };
        let signal = MediumTermMomentum::new(config);
        assert_eq!(signal.lookback_days(), 150);
    }

    #[test]
    fn test_default_signal() {
        let signal = MediumTermMomentum::default();
        assert_eq!(signal.lookback_days(), 126);
    }
}
