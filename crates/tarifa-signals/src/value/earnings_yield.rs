...
/// Configuration for earnings yield signal.
///
/// This signal computes the ratio of earnings to market capitalization.
/// Higher values indicate stocks with high earnings relative to price.
...
/// Earnings yield value signal.
///
/// Computes the ratio of earnings to market capitalization (E/P ratio).
/// This is the inverse of the P/E ratio and serves as a value signal where
/// higher values indicate potentially undervalued stocks.
///
/// The signal applies:
/// 1. Earnings-to-price ratio calculation
...
///
/// ```ignore
/// use tarifa_signals::value::EarningsYield;
///
/// let signal = EarningsYield::default();
...
impl EarningsYield {
    /// Create a new earnings yield signal with the given configuration.
    #[must_use]
    pub const fn new(config: EarningsYieldConfig) -> Self {
        Self { config }
    }
    ...
    /// Check if using trailing twelve months earnings.
    #[must_use]
...
impl Default for EarningsYield {
    fn default() -> Self {
        Self::new(EarningsYieldConfig::default())
    }
}

impl Signal for EarningsYield {
    fn name(&self) -> &str {
        "earnings_yield"
    }
    ...
    fn score(&self, data: &MarketData, date: Date) -> Result<DataFrame> {
        ...
        for symbol in &unique_symbols {
            ...
            // Get most recent earnings and market cap
            let earnings = sorted
                .column("earnings")?
                .as_materialized_series()
                .f64()?
            ...
            match (earnings, market_cap) {
                (Some(e), Some(mc)) if mc > 0.0 && e.is_finite() => {
                    let ratio = e / mc;
        ...
        if result_symbols.is_empty() {
            return Err(TarifaError::InsufficientData(
                "No symbols have valid earnings and market cap data".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_default_config() {
        let config = EarningsYieldConfig::default();
    ...
    fn test_custom_config() {
        let config = EarningsYieldConfig {
            winsorize: false,
            winsorize_pct: 0.05,
            use_ttm: false,
        };
        let signal = EarningsYield::new(config);
