...
impl Default for ReturnOnAssetsConfig {
    fn default() -> Self {
        Self {
            winsorize: true,
            winsorize_pct: 0.01,
...
/// Return on assets quality signal.
///
/// Computes ROA as net income divided by total assets.
/// Higher values indicate more efficient asset utilization.
///
/// The signal applies:
/// 1. ROA calculation (net_income / total_assets)
...
///
/// ```ignore
/// use tarifa_signals::quality::ReturnOnAssets;
///
/// let signal = ReturnOnAssets::default();
/// let scores = signal.score(&market_data, date)?;
/// ```
#[derive(Debug, Clone)]
pub struct ReturnOnAssets {
    config: ReturnOnAssetsConfig,
}

impl ReturnOnAssets {
    /// Create a new return on assets signal with the given configuration.
    #[must_use]
    pub const fn new(config: ReturnOnAssetsConfig) -> Self {
        Self { config }
    }
...
impl Default for ReturnOnAssets {
    fn default() -> Self {
        Self::new(ReturnOnAssetsConfig::default())
    }
}

impl Signal for ReturnOnAssets {
    fn name(&self) -> &str {
        "return_on_assets"
    }
    ...
    fn score(&self, data: &MarketData, date: Date) -> Result<DataFrame> {
        ...
        for symbol in &unique_symbols {
            ...
            // Get most recent net income and total assets
            ...
            match (net_income, total_assets) {
                (Some(ni), Some(ta)) if ta.abs() > 1e-10 && ni.is_finite() => {
                    let roa = ni / ta;
                    result_symbols.push(symbol.clone());
                    result_ratios.push(roa);
                }
                _ => continue,
            }
        }
        ...
        if result_symbols.is_empty() {
            return Err(TarifaError::InsufficientData(
                "No symbols have valid net income and total assets data".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_default_config() {
        let config = ReturnOnAssetsConfig::default();
    ...
    fn test_custom_config() {
        let config = ReturnOnAssetsConfig {
            winsorize: false,
            winsorize_pct: 0.05,
            use_ttm: false,
        };
        let signal = ReturnOnAssets::new(config);
