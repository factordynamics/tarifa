...
impl Default for ReturnOnEquityConfig {
    fn default() -> Self {
        Self {
            winsorize: true,
            winsorize_pct: 0.01,
...
/// Return on equity quality signal.
///
/// Computes ROE as net income divided by shareholder equity.
/// Higher values indicate more efficient use of shareholder capital.
///
/// The signal applies:
/// 1. ROE calculation (net_income / total_equity)
...
///
/// ```ignore
/// use tarifa_signals::quality::ReturnOnEquity;
///
/// let signal = ReturnOnEquity::default();
/// let scores = signal.score(&market_data, date)?;
/// ```
#[derive(Debug, Clone)]
pub struct ReturnOnEquity {
    config: ReturnOnEquityConfig,
}

impl ReturnOnEquity {
    /// Create a new return on equity signal with the given configuration.
    #[must_use]
    pub const fn new(config: ReturnOnEquityConfig) -> Self {
        Self { config }
    }
...
impl Default for ReturnOnEquity {
    fn default() -> Self {
        Self::new(ReturnOnEquityConfig::default())
    }
}

impl Signal for ReturnOnEquity {
    fn name(&self) -> &str {
        "return_on_equity"
    }
    ...
    fn score(&self, data: &MarketData, date: Date) -> Result<DataFrame> {
        ...
        for symbol in &unique_symbols {
            ...
            // Get most recent net income and total equity
            let net_income = sorted
                .column("net_income")?
                .as_materialized_series()
                .f64()?
                .get(0);

            let total_equity = sorted
                .column("total_equity")?
                .as_materialized_series()
                .f64()?
                .get(0);
            ...
            match (net_income, total_equity) {
                (Some(ni), Some(eq)) if eq.abs() > 1e-10 && ni.is_finite() => {
                    let roe = ni / eq;
                    result_symbols.push(symbol.clone());
                    result_ratios.push(roe);
                }
                _ => continue,
            }
        }
        ...
        if result_symbols.is_empty() {
            return Err(TarifaError::InsufficientData(
                "No symbols have valid net income and equity data".to_string(),
            ));
        }
    ...
    fn required_columns(&self) -> &[&str] {
        &["symbol", "date", "net_income", "total_equity"]
    }
}
...
mod tests {
    ...
    fn test_default_config() {
        let config = ReturnOnEquityConfig::default();
    ...
    fn test_custom_config() {
        let config = ReturnOnEquityConfig {
            winsorize: false,
            winsorize_pct: 0.05,
            use_ttm: false,
        };
        let signal = ReturnOnEquity::new(config);
